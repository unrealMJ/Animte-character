...
class IPAdapter(torch.nn.Module):
    """IP-Adapter"""
    def __init__(self, unet, image_proj_model, adapter_modules, embedding=None):
        super().__init__()
        self.unet = unet
...
def main():
    ...
    # optimizer
    params_to_opt = itertools.chain(ip_adapter.adapter_modules.parameters(), ip_adapter.image_proj_model.parameters())
    ...
    for step in range(begin_step, args.num_train_steps):
        ...
        with accelerator.accumulate(ip_adapter):
            ...
            with torch.no_grad():
                image_embeds = image_encoder(batch["clip_images"].to(accelerator.device, dtype=weight_dtype)).last_hidden_state
