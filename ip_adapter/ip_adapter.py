...
class ImageProjModel(torch.nn.Module):
    """Projection Model"""
    def __init__(self, cross_attention_dim=768, clip_embeddings_dim=1024):
        super().__init__()
    ...
    def forward(self, last_hidden_state):
        # last_hidden_states  # [b, L, 1024]
        clip_proj_fea = self.proj(last_hidden_state)  # [b, L, 768]
        clip_proj_fea = self.norm(clip_proj_fea)
        return clip_proj_fea
...
class IPAdapter:
    ...
    def get_image_embeds(self, pil_image):
        ...
        last_hidden_state = self.image_encoder(clip_image.to(self.device, dtype=torch.float16)).last_hidden_state
        last_hidden_state = last_hidden_state[:, 1:, :]
        image_prompt_embeds = self.image_proj_model(last_hidden_state)
        uncond_image_prompt_embeds = self.image_proj_model(torch.zeros_like(last_hidden_state))
