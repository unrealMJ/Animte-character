...
The __call__ has `image` and `control_image` parameter, the `image` is the image prompt input to clip image encoder
'''
...
class StableDiffusionImageVariationControlNetPipeline(DiffusionPipeline):
    r"""
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if guess_mode and self.do_classifier_free_guidance:
                    ...
                    controlnet_prompt_embeds = image_embeddings.chunk(2)[1]
                else:
