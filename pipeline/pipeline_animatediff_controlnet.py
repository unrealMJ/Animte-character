...
class AnimateDiffControlNetPipeline(DiffusionPipeline, TextualInversionLoaderMixin, IPAdapterMixin, LoraLoaderMixin):
    ...
    def __init__(
        ...
        # TODO: check do_conver_RGB parameter
    ...
    def prepare_image(
        ...
        video = self.control_image_processor.preprocess(video, height=height, width=width).to(dtype=torch.float16) # (f, 3, h, w)
        video = video[None] # (b, f, 3, h, w) b=1
    ...
    def __call__(
        ...
        if isinstance(controlnet, ControlNetModel):
            ...
            raise NotImplementedError("MultiControlNetModel not implemented yet")
        else:
            assert False
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                control_model_input = einops.rearrange(control_model_input, "b c f h w -> (b f) c h w")
                control_encoder_hidden_states = controlnet_prompt_embeds[:, None].repeat(1, num_frames, 1, 1)
