...
class Inference:
    ...
    def build_pipe(self, path):
        if self.cfg.control_type == 'canny':
            ...
            controlnet = ControlNetModel.from_pretrained(self.cfg.hed_controlnet)
        else:
    ...
    def save(self, grid, all_images, prompt, exp_dir, step):
        os.makedirs('results', exist_ok=True)
        prompt = prompt.replace('/', '')[:100]
        current_time = time.strftime("%Y-%m-%d-%H-%M-%S")
        grid.save(f'results/{prompt}_{current_time}.png')

        single_image_dir = os.path.join('results', current_time)
        os.makedirs(single_image_dir, exist_ok=True)
        for i in range(len(all_images)):
            all_images[i].save(os.path.join(single_image_dir, str(i) + '.png'))
    ...
    def single_image_infer(self, reference, prompt, control_image):
        ...
        results = self.pipe(prompt=prompt, width=width, height=height, num_inference_steps=50, generator=generator, num_images_per_prompt=4, image=control_image).images
...
if __name__ == '__main__':
    ...
    grid, all_images = infer.single_image_infer(reference, prompt, control_image)
    infer.save(grid, all_images, prompt, exp_dir, step)
